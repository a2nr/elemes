...
def test_quiz_retry_same_attempt_id_idempotent(client):
    payload = _payload()
    first = client.post("/api/lesson-progress", json=payload)
    assert first.status_code == 200
    assert first.get_json()["idempotent"] is False

    second = client.post("/api/lesson-progress", json=payload)
    assert second.status_code == 200
    assert second.get_json()["idempotent"] is True


def test_quiz_focus_lost_terminated_accepted(client):
    resp = client.post("/api/lesson-progress", json=_payload())
    assert resp.status_code == 200
    assert resp.get_json()["success"] is True
...
def test_quiz_invalid_status_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"status": "cheated"})
    )
    assert resp.status_code == 400
...
def test_quiz_invalid_reason_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"termination_reason": "minimize_app"})
    )
    assert resp.status_code == 400
...
def test_quiz_invalid_score_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"score": "9/4"})
    )
    assert resp.status_code == 400
...
def test_quiz_malformed_attempt_id_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"attempt_id": "bukan-uuid"})
    )
    assert resp.status_code == 400
...
def test_quiz_invalid_timestamp_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"occurred_at": "kemarin-sore"})
    )
    assert resp.status_code == 400
...
def test_quiz_negative_visibility_count_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"visibility_event_count": -1})
    )
    assert resp.status_code == 400
...
def test_quiz_malformed_json_rejected(client):
    resp = client.post(
        "/api/lesson-progress", data="{rusak", content_type="application/json"
    )
    assert resp.status_code == 400
...
def test_quiz_invalid_token_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"token": "TOKEN_SALAH"})
    )
    assert resp.status_code == 401
...
def test_quiz_unknown_lesson_rejected(client):
    resp = client.post(
        "/api/lesson-progress", json=_payload({"lesson_name": "tidak_ada"})
    )
    assert resp.status_code == 404
...
# ── GET /api/lesson-progress/<lesson_name> (review-after-refresh) ──

def test_get_progress_requires_token(client):
    resp = client.get(f"/api/lesson-progress/{LESSON}")
    assert resp.status_code == 400


def test_get_progress_invalid_token_rejected(client):
    resp = client.get(f"/api/lesson-progress/{LESSON}?token=TOKEN_SALAH")
    assert resp.status_code == 401


def test_get_progress_unknown_lesson_rejected(client):
    resp = client.get("/api/lesson-progress/tidak_ada?token=" + STUDENT_TOKEN)
    assert resp.status_code == 404


def test_get_progress_returns_audit_after_quiz(client):
    payload = _payload({"status": "submitted", "termination_reason": None, "score": "3/4"})
    client.post("/api/lesson-progress", json=payload)

    resp = client.get(f"/api/lesson-progress/{LESSON}?token={STUDENT_TOKEN}")
    assert resp.status_code == 200
    body = resp.get_json()
    assert body["success"] is True
